      ...
      <nav>
        ...
        <div class="pt-4" v-for="(workspace, windex) in workspaces" :key="workspace.owner + workspace.repo + workspace.branch" style="white-space: nowrap">
          <div>
            <button
              type="submit"
              class="font-bold rounded cursor-pointer pr-1"
              @click="clearWorkspace({ workspaceId: windex })"
          ...
          <div class="px-4" v-for="(file, findex) in workspace.files" :key="file.path">
            <button
              type="submit"
              class="font-bold rounded pr-1"
              @click="clearFile({ workspaceId: windex, fileId: findex })"
            ...
            <a
              ...
              @click="selectFileForEditor({ workspaceId: windex, fileId: findex })"
              :title="file.path">{{file.path}}</a><br>
          </div>
        </div>
        <div class="pt-4" v-for="(scratch, sindex) in scratches" :key="scratch.name" style="white-space: nowrap">
          <button
            type="submit"
            class="font-bold rounded cursor-pointer pr-1"
            @click="leaveScratch({ scratchId: sindex })"
          ...
          <form class="absolute z-50 fixed top-0 left-0 rounded p-2 border bg-white shadow" v-if="scratchUserNameOpen" @keydown.enter.prevent="save">
            ...
            <div class="flex items-center justify-between">
              <button
                ...
                type="button"
                :class="{
                  'bg-gray-500': scratchUserNameForm.trim().length === 0, 'bg-blue-500': scratchUserNameForm.trim().length !== 0, 'hover:bg-blue-700': scratchUserNameForm.trim().length !== 0, 'cursor-not-allowed': scratchUserNameForm.trim().length === 0,
                }"
                @click="save"
  ...
  computed: {
    ...
    activefile() {
      if (this.$store.state.activeWorkspace === undefined
          || this.$store.state.activeWorkspace >= this.$store.state.workspaces.length
          || this.$store.state.activeWorkspace < 0) {
        return undefined;
      }
